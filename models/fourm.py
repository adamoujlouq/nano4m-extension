...
class FourM(nn.Module):
    ...
    def forward_encoder(
        ...
        # Embed input tokens: [B, N, D]
        x = self.enc_tok_emb(enc_input_tokens)

        # Add modality embeddings: [B, N, D]
    ...
    def forward_decoder(
        ...
        # Embed target modality IDs: [B, M, D]
    ...
    def generate_one_modality_roar(
        ...
        # Randomly shuffle positions 0..n_tokens_target-1 and split into per-step chunks
        shuffled_positions = torch.randperm(n_tokens_target, device=device)
        dec_input_positions_list = []
        idx = 0
        for k in schedule:
            positions = shuffled_positions[idx: idx + k].unsqueeze(0)  # [1, k]
            dec_input_positions_list.append(positions)
            idx += k
        
        for step, k in enumerate(schedule):
            # Select the k positions to predict for this step
            dec_input_positions = dec_input_positions_list[step]  # [1, k]
            ...
            # Forward pass to get logits, select batch element 0: [k, vocab_size]
            ...
            # samples shape: [k], unsqueeze to [1, k]
            samples = samples.unsqueeze(0)

            # Concatenate new tokens to encoder inputs for the next step
